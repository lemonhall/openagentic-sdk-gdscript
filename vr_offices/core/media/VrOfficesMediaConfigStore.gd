extends RefCounted
class_name VrOfficesMediaConfigStore
...
static func config_path(save_id: String) -> String:
	...
	if sid == "":
		return ""
	return "%s/vr_offices/media_config.json" % _OAPaths.save_root(sid)
...
static func load_config(save_id: String) -> Dictionary:
	...
	var token := String(obj.get("bearer_token", obj.get("bearerToken", ""))).strip_edges()
	if base_url.ends_with("/"):
		base_url = base_url.rstrip("/")
	return {"ok": true, "config": {"base_url": base_url, "bearer_token": token}}
...
static func save_config(save_id: String, config: Dictionary) -> Dictionary:
	...
	var token := String(config.get("bearer_token", config.get("bearerToken", ""))).strip_edges()
	if base_url.ends_with("/"):
		base_url = base_url.rstrip("/")
	...
	f.store_string(JSON.stringify({"base_url": base_url, "bearer_token": token}, "  ") + "\n")
	f.close()
