...
func _init() -> void:
	...
	if not T.require_true(self, sent.any(func(x: String) -> bool: return x == "AUTHENTICATE PLAIN"), "Expected AUTHENTICATE PLAIN"):
		return

	fake.server_push_line("AUTHENTICATE +")
	client.call("poll")
	await process_frame
	var expected_payload := "AUTHENTICATE %s" % _sasl_plain_b64("u", "p")
	for l in (buf.call("push_chunk", fake.server_take_outbound_text()) as Array[String]):
		sent.append(l)
	if not T.require_true(self, sent.any(func(x: String) -> bool: return x == expected_payload), "Expected SASL payload"):
		return

	# SASL fail numeric (still end CAP and proceed).
	fake.server_push_line(":srv 904 nick_test :SASL fail")
	...
	if not T.require_true(self, sent.any(func(x: String) -> bool: return x == "CAP END"), "Expected CAP END after SASL fail"):
		return

	var idx_end: int = sent.find("CAP END")
	var idx_payload: int = sent.find(expected_payload)
