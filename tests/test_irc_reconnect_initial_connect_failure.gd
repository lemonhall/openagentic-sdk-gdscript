...
class FakePeerFactory:
	...
	func make_peer(host: String, port: int) -> Object:
		...
		# First connect attempt fails before ever reaching STATUS_CONNECTED.
		p.status = StreamPeerTCP.STATUS_ERROR if peers.is_empty() else StreamPeerTCP.STATUS_CONNECTED
		peers.append(p)
		return p
...
func _init() -> void:
	...
	# First poll sees STATUS_ERROR and must schedule reconnect.
	client.call("poll", 0.0)
	await process_frame

	# Next poll should perform the reconnect attempt immediately (backoff 0.0).
	client.call("poll", 0.0)
	await process_frame
	if not T.require_eq(self, factory.peers.size(), 2, "Expected reconnect attempt after initial connect failure"):
		return
