...
func _init() -> void:
	...
	if not T.require_true(self, sent.any(func(x: String) -> bool: return x == "CAP LS 302"), "Expected CAP LS 302 on start"):
		return
	if not T.require_true(self, not sent.any(func(x: String) -> bool: return x.begins_with("NICK ")), "NICK must not be sent before CAP END"):
		return
	if not T.require_true(self, not sent.any(func(x: String) -> bool: return x.begins_with("USER ")), "USER must not be sent before CAP END"):
		return
	...
	for i in range(sent.size()):
		...
		if idx_user == -1 and sent[i].begins_with("USER "):
			idx_user = i
	if not T.require_true(self, idx_end != -1 and idx_nick != -1 and idx_user != -1, "Expected CAP END + NICK + USER"):
		return
	if not T.require_true(self, idx_end < idx_nick and idx_end < idx_user, "Registration must occur after CAP END"):
		return
