...
### In-chat attachment UI（玩家在 NPC 聊天框里直接发送）

（这是你指出的关键缺口：需要“在聊天界面里直接发送”，而不是让用户去外部脚本生成 `OAMEDIA1` 再粘贴。）

- **REQ-010** `DialogueOverlay` 必须提供“附件发送”能力（玩家侧）：
  - 有一个明显的“Attach/附件”按钮；
  - 支持一次选择**多个文件**（多选 FileDialog）；
  - 支持**拖拽**一个或多个文件到聊天窗口发送；
  - 发送过程必须是队列化的（多个文件按顺序上传/发送，不阻塞 UI 主线程）。
- **REQ-011** 上传/发送的 UX（最小可用）：
  - 每个文件显示：文件名、类型、大小、上传进度（% 或进度条）、状态（pending/uploading/sent/failed/cancelled）；
  - 支持取消“单个文件”与“全部队列”；
  - 失败时给出清晰错误（例如：不支持的类型、超过大小限制、网络错误、鉴权失败）。
- **REQ-012** 安全与隐私（玩家侧）：
  - 聊天里发送的内容只能是 `OAMEDIA1 ...` / `OAMEDIA1F ...`，**不得包含 bearer token**；
...
6) 玩家在 `DialogueOverlay` 中可以通过“Attach/拖拽”一次发送多个图片文件：队列、进度、取消、失败提示齐全；并且发送出的聊天内容不包含 token 或本机绝对路径。
