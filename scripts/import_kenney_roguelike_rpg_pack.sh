...
ZIP_PATH="${1:-$ROOT_DIR/kenney_roguelike-rpg-pack.zip}"
OUT_DIR="${2:-$ROOT_DIR/assets/kenney/roguelike-rpg-pack}"

if [[ ! -f "$ZIP_PATH" ]]; then
  echo "Missing zip: $ZIP_PATH" >&2
  echo "Usage: scripts/import_kenney_roguelike_rpg_pack.sh [zip_path] [out_dir]" >&2
  exit 2
fi
